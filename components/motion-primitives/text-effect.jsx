  ...
  const content =
    per === 'line' ? (
      ...
      <motion.span className='inline-block whitespace-pre'>
        {String(segment).split('').map((char, charIndex) => (
          <motion.span
            key={`char-${charIndex}`}
            aria-hidden='true'
            variants={variants}
...
const splitText = (text, per) => {
  ...
  if (per === 'line') return textString.split('\n');
  return textString.split(/(\s+)/);
};
    ...
    <AnimatePresence mode='wait'>
      {trigger && (
        <MotionTag
          ...
          style={style}
        >
          {per !== 'line' ? <span className='sr-only'>{String(children)}</span> : null}
